...
int main()
{
	...
	Physics phys;
	phys.MakeCube(reactphysics3d::Vector3(-15, 11.0f, -15), reactphysics3d::Vector3(1, 1, 1));
	...
	while (!glfwWindowShouldClose(window)) //(Checks if you've prompted closing out the window. (One example would be "X-ing out the window".))
	{
		...
		if (glfwGetKey(window, GLFW_KEY_C) == GLFW_PRESS&&!c_lock) { 
			phys.MakeCube(reactphysics3d::Vector3((viewCam.Position+(viewCam.Orientation*10.0f)).x, (viewCam.Position + (viewCam.Orientation * 10.0f)).y, (viewCam.Position + (viewCam.Orientation * 10.0f)).z), reactphysics3d::Vector3(0.5, 0.5, 0.5));
			phys.GetNewMeshes(physMeshes);
			phys.GetTransfrom(physTrans);
			phys.GetRotation(physRot);
			c_lock = true;
